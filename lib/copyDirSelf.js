/**
 * created by dkviurs at 2017年8月30日17:57:37
 */
const fs = require('fs');
 ...
 * @param dest 目标文件夹
 */
function copyDirSelf (src, dest, isreplace = false) {
    ...
    if (!fs.existsSync(path.join(dest, path.basename(src)))) {
        // 不存在，创建文件夹
        fs.mkdirSync(path.join(dest, path.basename(src)));
        dest = path.join(dest, path.basename(src));
    }

    // src = assets/test
    // dest = demo/test 目录已创建
    ...
    fs.readdir(src, function (err, files) {
        ...
        files.forEach(function (file) {
            ...
            if (fileStats.isFile()) {
                ...
                // srcPath = assets/test/test2
                // destPath = demo/test  目录已创建

                // const destPath = path.join(dest, file);
                // 目标目录下创建子目录，进行递归
                // fs.mkdirSync(destPath);
                copyDirSelf(srcPath, dest);
            }
        })
    });
}
